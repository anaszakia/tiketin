@extends('layouts.app')

@section('title', 'Detail Event Ticket')

@section('content')

    <div class="d-flex justify-content-between align-items-center mb-6">
        <div>
            <h4 class="mb-0">Detail Event Ticket</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('event_tickets.index') }}">Event Tickets</a></li>
                    <li class="breadcrumb-item active">Detail</li>
                </ol>
            </nav>
        </div>
        <div class="d-flex gap-2">
            @if(can('event_tickets.edit'))
                <a href="{{ route('event_tickets.edit', $eventTicket) }}" class="btn btn-primary">
                    <i class="ti ti-edit me-1"></i> Edit
                </a>
            @endif
            <a href="{{ route('event_tickets.index') }}" class="btn btn-white">
                <i class="ti ti-arrow-left me-1"></i> Kembali
            </a>
        </div>
    </div>

    <div class="card card-lg">
        <div class="card-body">
            <h6 class="mb-4 text-muted text-uppercase" style="font-size:11px; letter-spacing:.05em;">
                Informasi Event Ticket
            </h6>
            <table class="table table-borderless mb-0">
                        <tr>
                            <td class="text-muted" width="200">Event Id</td>
                            <td>{{ $eventTicket->event->name ?? '-' }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted" width="200">Name</td>
                            <td>{{ $eventTicket->name ?? '-' }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted" width="200">Description</td>
                            <td>{{ $eventTicket->description ?? '-' }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted" width="200">Price</td>
                            <td>{{ $eventTicket->price ?? '-' }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted" width="200">Quota</td>
                            <td>{{ $eventTicket->quota ?? '-' }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted" width="200">Sales Start</td>
                            <td>{{ $eventTicket->sales_start ?? '-' }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted" width="200">Sales End</td>
                            <td>{{ $eventTicket->sales_end ?? '-' }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted" width="200">Status</td>
                            <td><span class="badge bg-secondary">{{ $eventTicket->status }}</span></td>
                        </tr>
                        <tr>
                            <td class="text-muted">Dibuat</td>
                            <td>{{ tgl_jam($eventTicket->created_at) }}</td>
                        </tr>
                        <tr>
                            <td class="text-muted">Terakhir Update</td>
                            <td>{{ tgl_jam($eventTicket->updated_at) }}</td>
                        </tr>
            </table>
        </div>
    </div>

@endsection